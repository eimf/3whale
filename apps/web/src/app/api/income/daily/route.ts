 ...
 * Query: days= or from=&to= (YYYY-MM-DD), optional granularity=hour|day, optional compare=1.
...
export async function GET(request: Request) {
  ...
  if (!hasDays && !hasFromTo) {
    return NextResponse.json(
      {
        error:
          "Provide either days (1|2|3|7|14|30|90|365) or both from and to (YYYY-MM-DD)",
      },
      { status: 400 }
    );
  }
