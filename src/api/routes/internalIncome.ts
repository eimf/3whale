...
export async function registerInternalIncomeRoutes(fastify: FastifyInstance) {
    ...
    fastify.get<{
        ...
        if (rawRow?.payload) {
            ...
            logger.warn(
                { shopifyOrderId },
                "shopify_order_raw missing for order",
            );
        }
    ...
    /** GET /internal/income/reconcile-parity */
    fastify.get<{
        Querystring: Record<string, string | undefined>;
    }>("/internal/income/reconcile-parity", async (req, reply) => {
        const parsed = reconcileParityQuerySchema.safeParse(req.query);
        if (!parsed.success) {
            return reply.code(400).send({
                error: "Invalid query",
                details: parsed.error.flatten(),
        ...
        const result = await getReconcileParity({
            startUtc: range.startUtc,
            endUtc: range.endUtc,
            includeExcluded: q.includeExcluded,
            limit: q.limit,
