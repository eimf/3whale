...
export function computeIncomeComponents(
  ...
  const parsedRefunds = refunds.map((r) => ShopifyRefundNormalizedSchema.parse(r));
  ...
  const refundsSum =
    parsedRefunds.length === 0
      ...
      : parsedRefunds.reduce((acc, r) => addMoney(acc, r.amount), zeroMoney(currencyCode));

  // income_bruto = subtotal (after discounts) + shipping; tax excluded by design
  const income_bruto = addMoney(parsedOrder.lineItemsSubtotal, parsedOrder.shippingAmount);
  const income_neto = subMoney(income_bruto, refundsSum);

  return {
    currencyCode,
    line_items_subtotal: parsedOrder.lineItemsSubtotal,
