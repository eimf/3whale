...
import type { ShopifyOrderNormalized, ShopifyRefundNormalized } from "../shopifyOrderTypes";

const MXN = "MXN";
const iso = "2024-01-15T12:00:00.000Z";

function order(overrides: Partial<ShopifyOrderNormalized>): ShopifyOrderNormalized {
  return {
...
function refund(amount: string, id = "r1"): ShopifyRefundNormalized {
  return { id: `gid://shopify/Refund/${id}`, createdAt: iso, amount: { amount, currencyCode: MXN } };
}
...
describe("computeIncomeComponents", () => {
  ...
  it("currency mismatch throws", () => {
    ...
    expect(() => computeIncomeComponents(o, [refund("200")])).toThrow(/Currency mismatch/);
  });
});
...
describe("shouldExcludeOrderV1", () => {
  it("excludes when cancelled", () => {
    ...
    expect(shouldExcludeOrderV1(o, [])).toEqual({ exclude: true, reason: "cancelled" });
  });

  it("excludes when test", () => {
    const o = order({ isTest: true });
    expect(shouldExcludeOrderV1(o, [])).toEqual({ exclude: true, reason: "test" });
  });
  ...
  it("excludes when fully refunded (refunds = bruto)", () => {
    ...
    expect(shouldExcludeOrderV1(o, refs)).toEqual({ exclude: true, reason: "fully_refunded" });
  });
  ...
  it("does not exclude when partial refund", () => {
    ...
    expect(shouldExcludeOrderV1(o, [refund("100")])).toEqual({ exclude: false });
  });
});
